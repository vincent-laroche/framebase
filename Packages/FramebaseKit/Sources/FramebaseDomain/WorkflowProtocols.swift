...
public protocol WorkflowRepository: Sendable {
    ...
    func approve(workflowRunID: UUID, currentSnapshot: WorkflowInputSnapshot, actor: WorkflowAuditActor, at date: Date) async throws -> WorkflowRun
    func executeApproved(workflowRunID: UUID, currentSnapshot: WorkflowInputSnapshot, actor: WorkflowAuditActor, at date: Date) async throws -> WorkflowRun
    /// Reverts only exact catalog effects stored by a completed workflow run.
    /// Calling undo again is idempotent and returns `false`.
    func undo(workflowRunID: UUID, actor: WorkflowAuditActor, at date: Date) async throws -> Bool
