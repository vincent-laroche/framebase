...
**Current source-only implementation (2026-08-10):** migration `0007_agent_operations.sql` and the `agents` Worker route implement the first narrow remote adapter without deploying it. A trusted device can mint a one-time returned, SHA-256-hashed delegated credential whose scopes are a subset of its own; only that device can revoke it. An active agent with both `assets.metadata.write` and `workflows.run` can propose adding an existing tag to up to 500 active assets at an exact catalog revision. The owner device issues a 15-minute opaque approval only if the full target/tag/membership snapshot is unchanged. The same shared idempotent mutation executor used by `/v1/mutations` then adds only previously missing memberships and preserves any pre-existing membership. Every identity/proposal/approval/apply/expiry/stale event is attributed. No migration has been applied, identity created, endpoint deployed, MCP server hosted, cloud credential emitted, or personal media touched.
...
- [ ] Prove scope denial, revocation, proposal expiry, audit attribution, and CLI/MCP parity. The individual local and source-only adapter tests cover scope denial, revocation, expiry, and attribution; `Cloud/contracts/agent-tag-proposal-parity-v1.json` is now read by the Swift CLI, HTTP contract, and MCP transport tests to pin the common tag-operation scopes, bounds, and safety invariants. A full live app/CLI/MCP execution parity proof remains deferred until the approved remote adapter can be exercised without a private library.
